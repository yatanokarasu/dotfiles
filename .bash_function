...
function 256colors {
    ...
    # Print Header
    printf "  %3s  %-17s  %-7s  %-16s  %-9s  %-9s\n" "No." "Color Name" "Hex" "RGB" "TermFG" "TermBG"
    echo "-------------------------------------------------------------------------"

    for i in $(seq 0 255); do
        IFS=$';' read name hex rgb <<<${COLORS[${i}]}
        printf "  %3d  %-17s  %-7s  %-16s  \x1b[38;05;${i}m38;05;%-3d\x1b[0m  \x1b[48;05;${i}m48;05;%-3d\x1b[0m\n" ${i} "${name}" "${hex}" "${rgb}" ${i} ${i}
    done
}


function re() {
    printf -v index "%02d" $1; shift
    local name=$1; shift
...
function re2() {
    ...
    rename "s/^[^\.]+\./${index}. ${name}./" index.mp4 subtitle*
}
